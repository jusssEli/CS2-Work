...
AST::AST(const AST& t) {
    ...
    for(AST* childAst : t.child){
        AST* copy = new AST (*childAst);
        child.push_back(copy);
    }
}
AST::~AST(){
    for(AST* childAst : child){
        delete childAst;
    }
}
void AST::swap(AST& rhs){
    nodes tnodeType = nodeType;
    nodeType = rhs.nodeType;
    rhs.nodeType = tnodeType;

    std::string tempTag = tag;
    tag = rhs.tag;
    rhs.tag = tempTag;

    std::string tempcTag = closeTag;
    closeTag = rhs.closeTag;
    rhs.closeTag = tempcTag;
